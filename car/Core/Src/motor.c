...
void car_run()
{
	...
	if(Error>50)
		...
		if(Error>250)
			Error=250;
	__HAL_TIM_SET_COMPARE(&htim1,TIM_CHANNEL_1,100+Error);
	__HAL_TIM_SET_COMPARE(&htim1,TIM_CHANNEL_4,100);//左轮
		}
	else if(Error<-50)
			{
		if(Error<-250)
		...
		__HAL_TIM_SET_COMPARE(&htim1,TIM_CHANNEL_4,100-Error);
			}
	else
	{
	__HAL_TIM_SET_COMPARE(&htim1,TIM_CHANNEL_1,100);
